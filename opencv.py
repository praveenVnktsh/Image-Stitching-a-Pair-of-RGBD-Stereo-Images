...
def execute(index1, index2, prevH):
    ...
    os.makedirs('outputs/' + str(imageSet) + '/opencv/', exist_ok = True)
    ...
    for depthLevel in range(1, m + 1 ):
        ...
        levelinfo = info[-1]
        # print(levelinfo['matches'])
        srcin = src[levelinfo['matches']]
        dstin = dst[levelinfo['matches']]
        H, status = cv2.findHomography(srcin, dstin, cv2.RANSAC, threshold)
    ...
    cv2.imwrite(f'outputs/{imageSet}/opencv/i{index1}_i{index2}_depth_{m}.png', blendedImage)
